...
--   * `workspace_members` is the only dependency. The other thing these
--     joins need -- `issues_workspace_id_key`, the UNIQUE (workspace_id, id)
--     on `issues` -- is DECLARED BY THIS FILE, immediately below, because no
--     earlier migration has a use for it. `issue_labels_issue_fk` and
...
-- `issues`, so (workspace_id, id) cannot repeat -- and not redundant as a
-- constraint: PostgreSQL lets a foreign key target only a uniquely-
-- constrained column SET. Without this, `issue_labels_issue_fk` and
-- `comments_issue_fk` below cannot be declared, and a label or a comment
-- could be attached to an issue in another tenant.
--
-- Declared here rather than in 006 because 006 has no use for it: its own
-- composite keys point at `workspace_members`, not at `issues`. It is
-- declared exactly once in the whole chain -- 010 references it and
-- deliberately does not declare it, since a duplicate constraint name is a
...
CREATE TABLE labels (
    ...
    -- `issues_workspace_id_key` this file declares on `issues`.
