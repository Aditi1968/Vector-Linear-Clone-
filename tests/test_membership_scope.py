...
# after this file. Both are derived from the id so every user a test makes is
# distinct without naming one, and both satisfy 003's CHECKs: a UUID renders
# lowercase, the address has exactly one @, and the hash carries the argon2id
# prefix the column demands. Neither is a credential -- nothing authenticates.
INSERT_USER_SQL = """
...
async def _build_schema(connection) -> None:
    ...
    """
    # Whatever is there, not a list of what was there when this was written:
    # 003 added `sessions`, whose foreign key to `users` made a named DROP
    # fail on a table this file has never heard of.
