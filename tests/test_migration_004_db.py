...
an operator would and then asks the server. 001, 002 and 003 are applied
first, 003 because `workspace_members.user_id` references the `users` it
creates.
...
# `users` gained NOT NULL email and password_hash in 003, which was written
# after this file. Both values are derived from the id so that every user a
# test makes is distinct without the test having to name one, and both are
# shaped to satisfy 003's CHECKs: the email is lowercase (a UUID renders
# lowercase) and has exactly one @, and the hash carries the argon2id prefix
# the column demands. Neither is a credential -- nothing here authenticates.
INSERT_USER_SQL = """
...
async def applied(postgres_dsn):
    """001, 002, 003, 004 -- then a workspace and two accounts.
    ...
    try:
        # Whatever is there, not a list of what was there when this was
        # written: 003 added `sessions`, whose foreign key to `users` made a
        # named DROP fail on a table this file has never heard of.
